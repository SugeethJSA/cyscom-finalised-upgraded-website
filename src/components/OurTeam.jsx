...
const OurTeam = () => {
  ...
  return (
    <div
      ref={containerRef}
      className="relative bg-[#050505] min-h-screen text-blue-50 pt-24 md:pt-32 px-4 md:px-8 lg:px-24 font-general"
    >
      ...
      <div className="hero-section flex flex-col md:flex-row gap-8 md:gap-16 mb-16 md:mb-32 items-center">
        <div className="title-wrapper md:w-1/2 flex flex-col justify-center order-2 md:order-1 relative z-10">
          <p className="subtitle text-cyan-400 font-mono tracking-[0.2em] text-sm md:text-base mb-4 uppercase">
            ...
            {(() => {
              ...
              return Object.entries(grouped).map(([dept, members]) => (
                <div key={dept} className="space-y-4">
                  ...
                  <div className="team-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                    {members.map((member, idx) => (
                      <CabinetCard key={`${dept}-${idx}`} member={member} />
                    ))}
                  </div>
                </div>
              ));
...
const CabinetCard = ({ member }) => {
  return (
    <div className="team-card group relative overflow-hidden rounded-2xl bg-[#0a0a0a] border border-white/5 transition-[transform,border-color,box-shadow] duration-500 ease-out hover:-translate-y-2 hover:border-cyan-400/30 hover:shadow-[0_18px_50px_-18px_rgba(34,211,238,0.28)] cursor-pointer">
      <div className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none z-10">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(34,211,238,0.18),transparent_45%)]"></div>
        <div className="absolute inset-[1px] rounded-2xl border border-cyan-300/20"></div>
      </div>

      <div className="relative aspect-[3/4] overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/15 via-transparent to-purple-500/15 opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-10 mix-blend-screen pointer-events-none"></div>
        <img
          src={member.img}
          alt={member.name}
          className="w-full h-full object-cover object-center transition-transform duration-700 ease-out group-hover:scale-110 group-hover:grayscale-0 grayscale-[15%]"
        />
      </div>

      <div className="absolute inset-x-0 bottom-0 p-5 md:p-6 bg-gradient-to-t from-black via-black/85 to-transparent z-20">
        <div className="transition-transform duration-500 group-hover:-translate-y-1">
          <h3 className="font-zentry text-xl md:text-2xl uppercase text-white mb-1 drop-shadow-md">
            {member.name}
          </h3>
          <p className="font-mono text-[11px] md:text-xs text-cyan-300 tracking-[0.15em] uppercase">
            {member.department}
          </p>
          {member.quote && (
            <p className="text-xs text-gray-300 italic mt-2 opacity-0 group-hover:opacity-100 transition-opacity duration-500 line-clamp-2">
              "{member.quote}"
            </p>
          )}
        </div>
