...
const TargetCursor = ({ targetSelector = '.cursor-target', spinDuration = 2, hideDefaultCursor = true }) => {
  ...
  useEffect(() => {
    ...
    const enterHandler = e => {
      ...
      if (resumeTimeout) {
        disabledKeyboardCursor();
      }

      function disabledKeyboardCursor() {
  ...
  return (
    <div ref={cursorRef} className="fixed top-0 left-0 w-0 h-0 pointer-events-none z-[9999] mix-blend-difference transform -translate-x-1/2 -translate-y-1/2 hidden md:block" style={{ willChange: 'transform' }}>
      <div
        ref={dotRef}
