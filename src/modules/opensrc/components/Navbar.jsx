...
const navItems = [
  { name: "Projects Hub", path: "/", isExternal: false },
  { name: "Leaderboard", path: "/leaderboard", isExternal: false },
  { name: "Certificates", path: "/locker", isExternal: false },
  { name: "Hall of Fame", path: "/hall-of-fame", isExternal: false },
  { name: "Legacy", path: "/legacy", isExternal: false }
];
...
const NavBar = () => {
  ...
  useEffect(() => {
    if (audioElementRef.current) {
      if (isAudioPlaying) {
        audioElementRef.current.play().catch(() => console.log("Audio play deferred until user interaction"));
      } else {
  ...
  return (
    <>
      <div
        ref={navContainerRef}
        className="sticky top-20 z-40 mx-auto w-11/12 max-w-4xl border border-blue-500/20 bg-black/60 backdrop-blur-md rounded-2xl h-14 transition-all duration-700 print:hidden mt-2"
      >
        <header className="absolute top-1/2 w-full -translate-y-1/2">
          <nav className="flex size-full items-center justify-center p-4">
            ...
            <button
              className="md:hidden z-50 flex flex-col gap-1.5 p-2 cursor-target absolute left-4"
              ...
              <div className="md:flex md:items-center md:gap-1">
                {navItems.map((item, index) => (
                  <a
                    ...
                    href={item.path}
                    className={clsx(
                      "nav-hover-btn scroll-smooth cursor-target",
      ...
      {/* Render Mobile Menu Drawer using React Portal */}
      {createPortal(
          ...
          <div className="flex flex-col items-center justify-center h-screen gap-8 px-8 text-center">
            {navItems.map((item, index) => (
              <a
                ...
                href={item.path}
                className={clsx(
                  "text-3xl font-zentry transition-colors cursor-target",
                  location.pathname === item.path ? "text-blue-400" : "text-blue-50 hover:text-blue-300"
                )}
                onClick={(e) => {
                  e.preventDefault();
